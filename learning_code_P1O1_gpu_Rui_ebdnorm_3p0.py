...
max_epoch = 30000
max_epoch_pretraining = 10000
# min_epoch = 100
min_epoch_proposed = 2000
# min_epoch_encdec = 500

# size_window = 7000  # -Rui, run in all epochs
# size_window_encdec = 7000  # -Rui, run in all epochs

# -Rui, path
datapath = "./dataset/"
resultpath = "./result/P1O1_Rui/P1O1_3p0_seed20_80_1_35_ebdnorm/"
...
# with open("./dataset/list_inx_P1O1_training.pkl", "wb") as f:
#     pickle.dump(rnd_list_training, f)
# f.close()
#
# with open("./dataset/list_inx_P1O1_validation.pkl", "wb") as f:
...
#
# loading
# with open(datapath + "list_inx_P1O1_training.pkl", "rb") as f:
#     rnd_list_training = pickle.load(f)
# f.close()
#
# with open(datapath + "list_inx_P1O1_validation.pkl", "rb") as f:
...
##################################################
# -Rui, add one more layer to each encoder/decoder
##################################################
...
class Storage(nn.Module):
    def __init__(self):
        ...
        self.saved_converged_AE_encoder = nn.Sequential(
            nn.Conv2d(num_embedding + 1, 64, 3, stride=1, padding=1),
            nn.PReLU(),
            nn.MaxPool2d(2),
            nn.Dropout(dp_rate),
...
# arr_minimum_losses = np.ones(size_window) * 10000
...
for epoch in range(max_epoch_pretraining):
    ...
    # -Rui, no loop
    # arr_minimum_losses[0:-1] = arr_minimum_losses[1:]
    # arr_minimum_losses[size_window - 1] = loss_validation
    ...
    # if minimum_loss < np.min(arr_minimum_losses) or epoch == max_epoch_pretraining - 1:
    if epoch == max_epoch_pretraining - 1:
        ...
        print(f'ratio:{loss_validation / minimum_loss}')
...
# arr_minimum_losses = np.ones(size_window) * 10000
# arr_minimum_losses_encdec = np.ones(size_window_encdec) * 10000
...
for epoch in range(max_epoch):
    ...
    if epoch > min_epoch_proposed:
        ...
        if epoch == max_epoch - 1:
            # if minimum_loss < np.min(arr_minimum_losses) or epoch == max_epoch - 1:
            check_overfitting = 1
    ...
    #         # -Rui, no loop
    #         arr_minimum_losses_encdec[0:-1] = arr_minimum_losses_encdec[1:]
    ...
    #             model_save.saved_converged_decoder.load_state_dict(copy.deepcopy(model_outer.decoder.state_dict()))
    #
    #         if minimum_loss_encdec < np.min(arr_minimum_losses_encdec) or epoch == max_epoch - 1:
    #             model_outer.encoder.load_state_dict(model_save.saved_converged_encoder.state_dict())
    #             model_outer.decoder.load_state_dict(model_save.saved_converged_decoder.state_dict())
    #             print("epoch for encdec convergence: ##############################", str(epoch + 1))
    #             print(str(loss_validation_encdec / minimum_loss_encdec))
...
with torch.no_grad():
    ...
    predicted_dataset = model_outer.decoder(intpla_ebd)
    # convert gain values in -1 ~ +1 range to original values in dB scale
    original_gain = (max_gain_dB - min_gain_dB) * (original[:, 0] + 1) / 2 + min_gain_dB
    original_gain_linear = 10 ** (original_gain / 10)

    predicted_gain = (max_gain_dB - min_gain_dB) * (predicted_dataset[:, 0] + 1) / 2 + min_gain_dB
    predicted_gain_linear = 10 ** (predicted_gain / 10)
    ####################
    ...
    MSE_cov = 2 * ((predicted_dataset[:, 1:num_input] - original[:, 1:num_input]) ** 2).sum(axis=1)
    MSE_cov2 = MSE_cov / num_BS_antenna ** 2
    ...
    # sum_SO_cov = np.sum(original[:, 1:num_input] ** 2) + num_BS_antenna * original.shape[0]
...
with open(resultpath + "predicted_dataset_O1_3p5(360x360).pkl", "wb") as f:
    pickle.dump(predicted_dataset.detach().cpu().numpy(), f)
f.close()
...
with open(resultpath + "img_original_gain_O1_3p5(360x360).pkl", "wb") as f:
    pickle.dump(img_original_gain.detach().cpu().numpy(), f)
f.close()
with open(resultpath + "img_predicted_gain_O1_3p5(360x360).pkl", "wb") as f:
    pickle.dump(img_predicted_gain.detach().cpu().numpy(), f)
f.close()
with open(resultpath + "img_RMSE_cov_O1_3p5(360x360).pkl", "wb") as f:
    pickle.dump(img_RMSE_cov.detach().cpu().numpy(), f)
f.close()
with open(resultpath + "img_RMSE_cov_dB_O1_3p5(360x360).pkl", "wb") as f:
    pickle.dump(img_RMSE_cov_dB.detach().cpu().numpy(), f)
f.close()
with open(resultpath + "img_RMSE_cov2_O1_3p5(360x360).pkl", "wb") as f:
    pickle.dump(img_RMSE_cov2.detach().cpu().numpy(), f)
f.close()
with open(resultpath + "img_RMSE_cov2_dB_O1_3p5(360x360).pkl", "wb") as f:
...
#     20 * math.log10(np.sqrt(sum_MSE_cov / (num_tot_row * num_each_row)))))
print('Average RMSE of normalized covariance in dB scale: ' + str((
    20 * (sum_MSE_cov2 / (num_tot_row * num_each_row)).sqrt().log10()).item()))
# print('NMSE of normalized covariance in dB scale: ' + str(
#     10 * math.log10(sum_MSE_cov / sum_SO_cov)))
